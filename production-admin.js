// RebataTrack Production Admin Console — Website Build 105
...
function setScope(scope){
  ...
  if($('adminTopbarEyebrow'))$('adminTopbarEyebrow').textContent=beta?'RebataTrack Beta Program':'RebataTrack Production';
  if(beta){
...
async function loadUsers(force=false){
  ...
  const data=await callProduction('users-list',{limit:500}); state.users=data.users||[]; renderUsers(); scheduleProductionExpiryRefresh();
}
...
function renderIdentities(){
  ...
  if($('productionIdentityTableBody'))$('productionIdentityTableBody').innerHTML=rows.map(group=>{
    const members=(group.members||[]).map(m=>`<button class="production-identity-member" data-production-user="${esc(m.uid)}" type="button"><strong>${esc(m.name||m.email||'RebataTrack user')}</strong><span>${esc(m.email||shortId(m.uid))}</span></button>`).join('');
...
function accessItemHtml(item,type){
  ...
  let detail='';
  if(type==='premium')detail=item.indefinite?'Indefinite complimentary RebataTrack+':`Expires ${fmtExactDateTime(item.expiresAt)}`;
...
function renderAccess(){
  ...
  if($('productionPremiumGrantList'))$('productionPremiumGrantList').innerHTML=p.length?p.map(x=>accessItemHtml(x,'premium')).join(''):'<div class="admin-empty-inline">No active complimentary RebataTrack+ grants.</div>';
...
function durationButtons(includeIndefinite=true){const options=[['1d','1 Day'],['1w','1 Week'],['14d','2 Weeks'],['1m','1 Month'],['3m','3 Months'],['6m','6 Months'],['1y','1 Year'],['custom','Custom']];if(includeIndefinite)options.push(['indefinite','Indefinite']);return options.map(([v,l])=>`<button class="production-duration-option${state.durationPreset===v?' is-active':''}" data-duration-preset="${v}" type="button">${l}</button>`).join('');}
...
function durationFields(includeIndefinite=true){return `<div class="production-duration-grid">${durationButtons(includeIndefinite)}</div><div class="production-custom-duration" id="productionCustomDuration" ${state.durationPreset==='custom'?'':'hidden'}><div class="beta-field"><label for="productionCustomAmount">Custom amount</label><input id="productionCustomAmount" min="1" max="3650" inputmode="numeric" type="number" value="30"/></div><div class="beta-field"><label for="productionCustomUnit">Unit</label><select id="productionCustomUnit"><option value="minutes">Minutes</option><option value="hours">Hours</option><option value="days">Days</option><option value="weeks">Weeks</option><option value="months">Months</option><option value="years">Years</option></select></div><div class="beta-field production-custom-exact"><label for="productionCustomExact">Or exact expiration date &amp; time</label><input id="productionCustomExact" type="datetime-local" step="1"/><small class="admin-email-change-help">Optional. If set, this exact local date/time overrides the amount and unit above.</small></div></div>`;}
...
function renderPremiumAction(){
  ...
  $('productionActionKicker').textContent='Complimentary RebataTrack+';$('productionActionTitle').textContent=grant?.active?'Manage RebataTrack+ Grant':'Grant RebataTrack+ Access';
  const currentStatus=currentAccessStatusHtml('Current Grant',grant);
  $('productionActionBody').innerHTML=`<div class="production-action-note"><strong>Account-scoped — follows this profile across all authorized devices.</strong> This creates a server-issued complimentary-access record for this Firebase UID only. The user will see it on every device they sign in with. It never transfers to a different account sharing the same device, and it does not forge or overwrite an App Store / Google Play subscription. <strong>Revoking this Admin grant never cancels or alters a real App Store / Google Play subscription.</strong> A new one-time in-app grant notice is created so the user is greeted on next open with the grant timeline and confetti where motion is allowed.</div>${currentStatus}${durationFields()}${reasonField('Example: Beta reward, support goodwill, promotion, internal testing')}<div class="production-action-footer"><button class="admin-secondary-button" data-production-cancel-action type="button">Cancel</button>${grant?.active?'<button class="production-action-button red" data-production-revoke="premium" type="button">Revoke Current Grant</button>':''}<button class="admin-primary-button" data-production-submit="premium" type="button">${grant?.active?'Replace Grant':'Grant RebataTrack+'}</button></div>`;
}
...
function renderTrialAction(){
  ...
  $('productionActionKicker').textContent='Trial Administration';$('productionActionTitle').textContent=current?.active?'Manage Trial Access':'Grant New Trial';
  const preferredDevice=d.user?.activeDeviceID||d.trial?.deviceId||d.user?.homeDeviceID||'';
  const opts=devices.map(x=>{
    const lastSeen=x.lastSeenAt?`Last login ${fmtDateTime(x.lastSeenAt)}`:'No login recorded';
    const tags=[x.deviceId===preferredDevice?' · Current/Active':x.isPrimary?' · Primary':'',x.trialOverrideActive?' · Admin trial active':'',` · ${lastSeen}`].filter(Boolean).join('');
    return `<option value="${esc(x.deviceId)}" ${x.deviceId===preferredDevice?'selected':''}>${esc(x.deviceName||x.deviceModel||x.platform||'Device')} · ${esc(shortId(x.deviceId))}${esc(tags)}</option>`;
  }).join('');
  const deviceNote=devices.length?'Select the device this account is currently using. Trial access is device-specific: the app reads the trial grant from the signed-in account\'s registered device ID, so pick the device the user last signed in on. Check the last-login time in the dropdown if unsure.':'No registered devices found for this account.';
  const currentStatus=currentAccessStatusHtml('Current Admin Trial',current);
  $('productionActionBody').innerHTML=`<div class="production-warning"><strong>Trial grants are device-scoped.</strong> This creates a separate Admin trial window for one specific device — it does not rewrite the original anti-abuse trial record. The app on <em>that device</em> will show a one-time confirmation with confetti on next open. To grant account-level RebataTrack+ access that follows the user across all their devices instead, use <strong>Grant RebataTrack+</strong>.</div>${currentStatus}<div class="beta-field"><label for="productionTrialDevice">Select Device <span class="admin-required-inline">*</span></label><p class="admin-field-hint">${esc(deviceNote)}</p><select id="productionTrialDevice">${opts||'<option value="">No devices available</option>'}</select></div>${durationFields(false)}${reasonField('Example: Support exception, review testing, verified trial reset request')}<div class="production-action-footer"><button class="admin-secondary-button" data-production-cancel-action type="button">Cancel</button>${current?.active?'<button class="production-action-button red" data-production-revoke="trial" type="button">End Current Trial Grant</button>':''}<button class="admin-primary-button" data-production-submit="trial" type="button" ${!devices.length?'disabled':''}>${current?.active?'Replace Trial Grant':'Grant New Trial'}</button></div>`;
}
...
async function submitAction(kind,button){
  ...
  try{
    ...
    if(kind==='trial'){action='trial-reissue';payload.deviceId=$('productionTrialDevice')?.value||'';Object.assign(payload,currentDurationPayload());}
    ...
    const celebrationGrant=kind==='premium'||kind==='review'||kind==='trial';
    const successMessage=reset?'Password-reset email sent.':celebrationGrant?'Grant saved. The user will be greeted on their next supported app open.':'Production access updated.';
    setActionMessage(successMessage,'success');showProdToast(reset?'Password-reset email sent to the user.':celebrationGrant?'Grant saved with next-open celebration.':'Production access updated.');
