// Rebatify Production Admin Console — Website Build 104
...
function renderTrialAction(){
  ...
  const opts=devices.map(x=>`<option value="${esc(x.deviceId)}" ${x.deviceId===preferredDevice?'selected':''}>${esc(x.deviceName||x.deviceModel||x.platform||'Device')} · ${esc(shortId(x.deviceId))}${x.deviceId===preferredDevice?' · Current/Active':x.isPrimary?' · Primary':''}${x.trialOverrideActive?' · Admin trial active':''}</option>`).join('');
  const currentStatus=currentAccessStatusHtml('Current Admin Trial',current);
  $('productionActionBody').innerHTML=`<div class="production-warning"><strong>Original trial history is preserved.</strong>This does not delete or rewrite the device’s original anti-abuse trial record. It creates a separate device-specific Admin trial window for the duration you choose. The app will show a one-time confirmation with confetti and the exact expiration on the next open/foreground.</div>${currentStatus}<div class="beta-field"><label for="productionTrialDevice">Device <span class="admin-required-inline">*</span></label><select id="productionTrialDevice">${opts}</select></div>${durationFields(false)}${reasonField('Example: Support exception, review testing, verified trial reset request')}<div class="production-action-footer"><button class="admin-secondary-button" data-production-cancel-action type="button">Cancel</button>${current?.active?'<button class="production-action-button red" data-production-revoke="trial" type="button">End Current Trial Grant</button>':''}<button class="admin-primary-button" data-production-submit="trial" type="button">${current?.active?'Replace Trial Grant':'Grant New Trial'}</button></div>`;
}
