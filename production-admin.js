// Rebatify Production Admin Console — Website Build 105
...
function renderUserDrawer(data){
  ...
  const plusText=pg?.active?(pg.indefinite?'Complimentary · Indefinite':`Complimentary · Until ${fmtExactDateTime(pg.expiresAt)}`):(u.premiumActive?`${u.premiumSource||'Store'} entitlement`:re?.active?`App Review access · ${re.source||'Active'}${re.expiresAt?` · Until ${fmtExactDateTime(re.expiresAt)}`:' · Indefinite'}`:'No Rebatify+ access');
